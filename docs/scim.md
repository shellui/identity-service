...
`{company_id}` is the company numeric primary key. `GET /api/v1/scim` returns this `base_url` for the selected company.
...
**Authentication:** `Authorization: Bearer <company-scim-token>` — token and URL company id must match the same company.
...
1. **Bearer token binding** — The `CompanyScimToken` row fixes the tenant. The URL company id must match that company; otherwise the response is **401** (no handler logic runs on a mismatched pair).
